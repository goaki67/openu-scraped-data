...
**Objective:**
Design a robust algorithm for an academic degree planner. The goal is to generate an optimal, semester-by-semester course sequence that satisfies all degree requirements in the shortest time possible while balancing workload and handling complex course relationships, specifically focusing on varied prerequisite strictness and credit overlaps.
...
2.  **Prerequisite AST (Abstract Syntax Tree):** A recursive structure for each course defining its requirements. Requirements are categorized into three levels of strictness:
    *   **Mandatory (`kabala` - תנאי קבלה):** **Hard Constraint.** The course cannot be taken unless these are satisfied.
    *   **Highly Suggested (`darush` - ידע קודם דרוש):** **Strong Heuristic.** The algorithm should prioritize taking these before the target course. If not taken, the algorithm should flag a "Strong Warning."
    *   **Recommended (`mumlats` - ידע קודם מומלץ):** **Weak Heuristic.** Suggestions that improve the learning experience but have lower priority than bottleneck courses.
...
The algorithm should be an evolution of a standard Topological Sort (Kahn's or DFS-based), modified to handle non-linear dependencies and heuristic preferences.
...
*   **Unlocked Potential (The Bottleneck Metric):** For every course, calculate its "Successor Weight"—the total number of future courses it unlocks (directly or indirectly) through the `kabala` tree.
...
1.  **Requirement Fulfillment (Hard Constraints):** 
    *   Only consider courses whose `kabala` AST evaluates to `True`.
2.  **Heuristic Weighting (Selection Logic):**
    ...
    *   **Suggested Prerequisites:** Add a weight bonus to courses whose `darush` or `mumlats` requirements are already met. Conversely, if the planner *must* schedule a course, prioritize scheduling its `darush` predecessors in earlier semesters.
...
*   **User-in-the-Loop:** How should the algorithm react if a user "pins" a specific course to a specific semester? It must re-validate the entire future path and flag any `kabala` violations or `darush` warnings.
...
2.  **The Rationale:** For each choice, a brief "why" (e.g., "Critical bottleneck," "Satisfies 'darush' for Course X").
3.  **Warning Flags:** Any potential issues, such as "Strong Warning: Course Y scheduled without its 'darush' (Prior Knowledge Required) course Z."

---

**Guidance for the AI:** 
Focus on the **Graph Theory** aspect. Treat the degree as a Directed Acyclic Graph (DAG) where some nodes are "Alternative" nodes (OR logic), some have "Inhibition" edges (Overlaps), and some edges represent different "Weights" (Kabala vs. Darush vs. Mumlats). Use a priority queue to manage the "ready" set of courses during the sort.
